...
#+END_SRC
   ...
   sets it to happen on each 20MB on startup.
...
#+END_SRC
   ...
   Prevent garbage collection on features that use the minibuffer.
...
#+END_SRC
   ...
   Change the font to be Menlo size 12. It works well on my MacBook Pro
...
*** C/C++
    ...
    When editing C/C++ source files, use Google's C/C++ style.

#+BEGIN_SRC emacs-lisp
  (jcazevedo/require-packages '(google-c-style))
...
#+END_SRC
    ...
    I don't really enjoy =ensime='s semantic highlighting, so this disables it
    by default.
