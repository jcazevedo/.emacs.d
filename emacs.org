...
#+END_SRC
   ...
   Change the font to be Inconsolata size 10. It works well on my MacBook Pro
   15-inch retina with the default resolution.

#+BEGIN_SRC emacs-lisp
  (defvar jcazevedo/default-font "Inconsolata-10")
...
*** Selection
    ...
    Emacs. Also use =helm-flx= for supposedly better sorting (I'm still testing
    if that is true). =helm-flx= should be loaded before Helm for better
    results.

#+BEGIN_SRC emacs-lisp
  (jcazevedo/require-packages '(helm-flx helm helm-projectile))
