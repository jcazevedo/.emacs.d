...
| <kbd>C-c r</kbd> | Renames the current buffer and, if the buffer is visiting a file, rename the file as well.                         |
| <kbd>C-c D</kbd> | Kills the current buffer and deletes the file it is visiting.                                                      |
| <kbd>C-x g</kbd> | Opens a Magit status buffer for the Git repository contaning the file or directory the current buffer is visiting. |
